...
g := 1.0;
h := htilde;
V := (m^2*modphi^~2 - g/2*(modphi)^4 + h/3*modphi^6);
numiters   := 70:
xmax       := 375:
xmax_f     := 10000;
mystepsize := 0.01:
...
for j from 1 to numiters do
  ...
  sys_ode := {
    ...
    diff(utilde(x), x) = piecewise(x = 0, (1/3)*(-gtilde(x)^2*ftilde(x) + ftilde(x) - ftilde(x)^3 + htilde*ftilde(x)^5),
                                         -(2/x)*utilde(x) - gtilde(x)^2*ftilde(x) + ftilde(x) - ftilde(x)^3 + htilde*ftilde(x)^5),
  ...
  if type(ftilde_vals(round(xmax/mystepsize)), undefined) then  # if solution diverges to infinity
      ...
      if utilde_vals(z)*utilde_vals(z+1) < 0 then
        ...
        if count = 2 then
          if ftilde_vals(z) > 0 then
            print(j, "Last val too low. Trying new ftilde0:", ftilde0);
            low := (low + high) / 2:
            ftilde0 := (low + high)/2:
            break:
          else
...
for i from 2 to numelems(ftilde_vals) - 1 do
  ...
  if (logdiff_last < 0 and logdiff > 0) or (Im(log(ftilde_vals(i+1))) <> 0) then
    if ftilde_vals(i)/ftilde_max < 0.01 then
      logpt_max := x_vals(i)*0.90;
      logpt_min := x_vals(i)*0.85;
      break;
    end if;
  end if;
...
if (relslope > 1e-7) or (logpt_max < 0) then
...
fit := LeastSquares(x_vals[ceil(logpt_min/mystepsize)..ceil(logpt_max/mystepsize)], map(log, x_vals[ceil(logpt_min/mystepsize)..ceil(logpt_max/mystepsize)].~ftilde_vals[ceil(logpt_min/mystepsize)..ceil(logpt_max/mystepsize)]), x);
fit_slope := diff(%, x);
...
sys_ode := {
  ...
  diff(utilde_f(x), x) = -(2/x)*utilde_f(x) - gtilde_f(x)^2*ftilde_f(x) + ftilde_f(x) - ftilde_f(x)^3 + htilde*ftilde_f(x)^5,
...
if etilde = 0 then
  ...
  A0tilde := (wtilde -~ gtilde_vals_all)/~etilde:
end if:
...
V_vals_all := (m^2*~ftilde_vals_all^~2 - g/2*~ftilde_vals_all^~4 + h/3*~ftilde_vals_all^~6):
...
if exportvar = 1 then
  if mystepsize - mystepsize_f = 0 then
    ...
    Digits := 15:
    params := [ ["e :=", e], ["w :=", w], ["m :=", m], ["mm :=", m], ["g :=", g], ["gg :=", g], ["h :=", h] ]:
