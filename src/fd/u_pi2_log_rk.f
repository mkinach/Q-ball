      subroutine u_pi2_log_rk(n_At,n_Ax,n_Ay,n_Az,n_chi,n_dxdX,n_dxdX2,n
     &_dydY,n_dydY2,n_dzdZ,n_dzdZ2,n_phi1,n_phi2,n_pi1,np1_pi2,x,y,z,Nx,
     ...
     #2, j, k)) / hx * n_dxdX2(i, j, k) + mu ** 2 * n_phi2(i, j, k) * lo
     #g(beta ** 2 * (n_phi1(i, j, k) ** 2 + n_phi2(i, j, k) ** 2) + 0.10
     #00000000000000D-99) + mu ** 2 * n_phi2(i, j, k) - 0.1D1 * cc * n_c
     #hi(i, j, k) ** 2 * n_phi2(i, j, k)
      res(i,j,k)=qb
      end do
      end do
      end do
