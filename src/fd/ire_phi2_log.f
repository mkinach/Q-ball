      subroutine ire_phi2_log(n_At,n_Ax,n_Ay,n_Az,n_chi,n_dxdX,n_dxdX2,n
     &_dydY,n_dydY2,n_dzdZ,n_dzdZ2,n_phi1,n_phi2,n_pi1,nm1_phi2,np1_phi2
     ...
     #2(i + 1, j, k)) / hx * n_dxdX2(i, j, k) - 0.1D1 * mu ** 2 * n_phi2
     #(i, j, k) * log(beta ** 2 * (n_phi1(i, j, k) ** 2 + n_phi2(i, j, k
     #) ** 2) + 0.1000000000000000D-99) - 0.1D1 * mu ** 2 * n_phi2(i, j,
     # k) + cc * n_chi(i, j, k) ** 2 * n_phi2(i, j, k)
      res = res + qb**2
      end do
      end do
      end do
